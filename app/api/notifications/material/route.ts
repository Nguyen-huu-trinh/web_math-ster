...
const schema = z.object({ resourceId: z.string().uuid(), requestId: z.string().uuid() });
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.rpc("publish_material_notification", { resource_id: parsed.data.resourceId, submission_id: parsed.data.requestId });
